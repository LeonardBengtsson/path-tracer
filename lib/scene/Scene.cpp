...
Aabb wrap_aabb(const std::span<std::unique_ptr<SceneObject>> objects) {
...
void Scene::init() {
    if constexpr (ENABLE_BVH_OPTIMIZATION) {
        ...
        aabb_bvh = std::make_unique<AabbBvh<std::unique_ptr<SceneObject>>>(std::span(objects), max_tree_height, min_leaf_size);
    }
}
